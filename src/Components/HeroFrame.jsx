...
const HeroFrame = ({func , heading , headingStyle , secondHeading , secondHeadingStyle , bgImage}) => {
  return (
    <section className="banner md:h-full lg:h-[800px] flex flex-col md:flex-row justify-center items-center lg:justify-center  md:p-[20px]">
        ...
        <div className="subHeading text-section flex flex-col gap-2 md:h-auto lg:h-[70%] lg:w-1/2 lg:items-center lg:justify-center ">
          <article>
            <h1 className="flex lg:gap-1 gap-1 flex-col">
              <span className={`mobh1 ${headingStyle} lg:text-4xl text-[25px] md:text-[40px] text-wrap text-white bg-[#333333] font-bold w-fit p-1 uppercase inline-block`}>
                {heading ? heading:"Best Interior Designer"}
              </span>
              <span className={`mobcen ${secondHeadingStyle} lg:text-4xl text-[20px] md:text-[30px] text-white bg-[#fd0000] font-bold w-fit p-1 uppercase inline-block `}>
        ...
        {/* Form Section */}
        <div className="form form-section flex flex-col sm:w-full md:w-1/2 gap-4 bg-white rounded-lg p-12 mt-6 lg:mt-0 lg:w-[30%] shadow-inner-black">
  ...
  <div className="frombtn flex justify-center">
    <Button content={"SPEAK TO US"} onClick={func} style={"bg-blue-500"} />
  </div>
</div>
